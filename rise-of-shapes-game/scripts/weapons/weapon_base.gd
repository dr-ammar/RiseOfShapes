...
@export var reload_time: float = 1.5 # الوقت اللازم للتعشيق بالثواني
@export var shoot_reload_block_time: float = 0.4 # الوقت المانع للتعشيق بعد الإطلاق
...
func _ready():
	...
	# حفظ الوضع الافتراضي للسلاح
	if weapon_sprite:
		default_position = weapon_sprite.position
		default_rotation = weapon_sprite.rotation
		
	# إعداد مؤقت الإطلاق
	setup_fire_timer()
	setup_reload_block_timer()
...
func setup_reload_block_timer():
	reload_block_timer = Timer.new()
	reload_block_timer.one_shot = true
	add_child(reload_block_timer)
...
func reload():
	# منع التعشيق إذا كان السلاح ممتلئاً أو جارٍ الإطلاق/التعشيق أو لم ينتهِ وقت منع التعشيق
	if is_reloading or not reload_block_timer.is_stopped(): return
	if current_ammo == max_ammo or current_reserve_ammo == 0: return
		
	reset_weapon_visuals()
	print("بدء التعشيق...")
	is_reloading = true
	can_shoot = false
...
func play_shoot_anim():
	# بدء مؤقت منع التعشيق
	if reload_block_timer:
		reload_block_timer.start(shoot_reload_block_time)
...
func reset_weapon_visuals():
	if weapon_sprite:
		weapon_sprite.stop()
		weapon_sprite.position = default_position
		weapon_sprite.rotation = default_rotation
